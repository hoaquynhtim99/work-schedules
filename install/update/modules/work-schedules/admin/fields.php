...
if ($nv_Request->isset_request('choicesql', 'post')) {
    ...
    $array_choicesql = array('module' => 'table', 'table' => 'column', 'column' => '');
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    $dataform['show_addedit'] = ($dataform['required']) ? 1 : $nv_Request->get_int('show_addedit', 'post', 0);
    $dataform['show_website'] = $nv_Request->get_int('show_website', 'post', 0);
    $dataform['show_manager'] = $nv_Request->get_int('show_manager', 'post', 0);
    ...
    if ($dataform['fid']) {
        ...
        if (in_array($dataform['field'], $field_not_allow)) {
            ...
            if ($stmt->fetchColumn()) {
                ...
                $_columns_array = $db->columns_array(NV_MOD_TABLE . '_rows');
                if (isset($_columns_array[$dataform['field']])) {
                    $error = $lang_module['field_error'];
                }
            }
        }
        ...
        if (empty($dataform['fid'])) {
            if ($dataform['max_length'] <= 4294967296 and !empty($dataform['field']) and !empty($dataform['title'])) {
                ...
                $sql = "INSERT INTO " . NV_MOD_TABLE . "_field (
                    ...
                    required, show_addedit, show_website,
                    show_manager, class, language, default_value
                ) VALUES (
                    ...
                    " . $dataform['required'] . ", " . $dataform['show_addedit'] . ", '" . $dataform['show_website'] . "',
                    " . $dataform['show_manager'] . ", :class, '" . serialize($language) . "', :default_value
                )";
                ...
                if ($dataform['fid']) {
                    ...
                    if ($dataform['field_type'] == 'number' or $dataform['field_type'] == 'date') {
                        $sql_field_type = "DOUBLE NOT NULL DEFAULT '" . $dataform['default_value'] . "'";
                    } elseif ($dataform['max_length'] <= 255) {
                        $sql_field_type = "VARCHAR( " . $dataform['max_length'] . " ) NOT NULL DEFAULT ''";
                    } elseif ($dataform['max_length'] <= 65536) {
                        //2^16 TEXT
                        $sql_field_type = 'TEXT NOT NULL';
                    } elseif ($dataform['max_length'] <= 16777216) {
                        //2^24 MEDIUMTEXT
                        $sql_field_type = 'MEDIUMTEXT NOT NULL';
                    } elseif ($dataform['max_length'] <= 4294967296) {
                        //2^32 LONGTEXT
                        $sql_field_type = 'LONGTEXT NOT NULL';
                    }
                    $save = $db->exec("ALTER TABLE " . NV_MOD_TABLE . "_rows ADD " . $dataform['field'] . " " . $sql_field_type);
                }
            }
        } elseif ($dataform['max_length'] <= 4294967296) {
            $sql = "UPDATE " . NV_MOD_TABLE . "_field SET";
            if ($text_fields == 1) {
                $sql .= " match_type='" . $dataform['match_type'] . "',
                match_regex='" . $dataform['match_regex'] . "', func_callback='" . $dataform['func_callback'] . "', ";
            }
            $sql .= " max_length=" . $dataform['max_length'] . ", min_length=" . $dataform['min_length'] . ",
                required = '" . $dataform['required'] . "',
                field_choices='" . $dataform['field_choices'] . "',
                sql_choices = '" . $dataform['sql_choices'] . "',
                show_addedit = '" . $dataform['show_addedit'] . "',
                show_website = '" . $dataform['show_website'] . "',
                show_manager = '" . $dataform['show_manager'] . "',
                class = :class,
            ...
            if ($save and $dataform['max_length'] != $dataform_old['max_length']) {
                ...
                $save = $db->exec("ALTER TABLE " . NV_MOD_TABLE . "_rows CHANGE " . $dataform_old['field'] . " " . $dataform_old['field'] . " " . $sql_field_type);
            }
        }
        if ($save) {
...
if ($nv_Request->isset_request('del', 'post')) {
    ...
    if ($fid and !empty($field)) {
        ...
        $query2 = 'ALTER TABLE ' . NV_MOD_TABLE . '_rows DROP ' . $field;
    ...
    if ($num) {
        foreach ($_rows as $row) {
            ...
            $xtpl->assign('ROW', array(
                ...
                'show_addedit' => ($row['show_addedit']) ? 'fa-check-square-o' : 'fa fa-square-o',
                'show_manager' => ($row['show_manager']) ? 'fa-check-square-o' : 'fa fa-square-o'
            ));
...
// Xuất nội dung thêm, sửa trường dữ liệu
...
if (!isset($dataform)) {
    if ($fid) {
        ...
        $dataform = array();
        $dataform['show_addedit'] = 1;
        $dataform['required'] = 0;
        $dataform['show_manager'] = 1;
        $dataform['show_website'] = 1;
...
$dataform['show_addedit'] = ($dataform['show_addedit']) ? ' checked="checked"' : '';
$dataform['show_manager'] = ($dataform['show_manager']) ? ' checked="checked"' : '';
$dataform['show_website'] = ($dataform['show_website']) ? ' checked="checked"' : '';

$xtpl->assign('CAPTIONFORM', ($fid) ? $lang_module['field_editcaption'] . ': ' . $dataform['fieldid'] : $lang_module['field_addcaption']);
